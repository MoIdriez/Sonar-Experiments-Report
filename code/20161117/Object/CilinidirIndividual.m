...
datadir = 'D:\Git\Data\Experiments\20161117\3 - Object\Cilinder\';
plotdir = 'D:\Git\Sonar Experiments Report\plots\20161117\Object\CilindirIndividual\';
...
for j = 1:size(locs, 2)    
    for i = 1:size(sonars, 2)   
        fileLocation = char(strcat(datadir, locs(j), '\', sonars(i), '\data.txt'));
        ...
        plotnames = {...
            char(strcat(locs(j), ' - ', sonars(i), ' - Raw individual results'))...
            char(strcat(locs(j), ' - ', sonars(i), ' - RunAvg individual results'))...
            char(strcat(locs(j), ' - ', sonars(i), ' - Combine seperate directional results'))...
            char(strcat(locs(j), ' - ', sonars(i), ' - Combine seperate directional results with mean'))...
            char(strcat(locs(j), ' - ', sonars(i), ' - Combine all directional results with mean'))...
            char(strcat(locs(j), ' - ', sonars(i), ' - Combine seperate directional results with mean and standard deviation'))...
            };
